...
function printHelp() {
    ...
    w('  -crlf         Use Windows style CRLF for line breaks as opposed to LF');
...
function main(argv) {
    ...
    if (opts.group && opts.array && opts.outputMode !== OM_JSON) {
        ...
        getInput(opts, function (err, content, filename) {
            ...
            parseChunk(content, undefined, filename, true, headers.join(''), opts.lineBreak);
        });
    } else {
        // not streaming
        getInput(opts, function (err, buffer, filename) {
            ...
            parseChunk(buffer, null, filename, false, opts.lineBreak);
        });
    }
     ...
     */
    function parseChunk(chunk, obj, filename, inPlace, headers, lineBreak) {
